...
export default function CompanyVerificationStatus({ onVerificationUpdate }: CompanyVerificationStatusProps) {
  ...
  const fetchVerificationStatus = async () => {
    try {
      ...
      if (!token) {
        console.error('No access token found for company');
        alert('Authentication error: Please log in again');
        return;
      }

      console.log('Fetching company verification status...');
  ...
  const handleSubmitDocument = async () => {
    ...
    try {
      ...
      formData.append('folderPath', 'verification-documents/companies');
  ...
  if (!verificationData) {
    return (
      <div className="space-y-6">
        ...
        <VerificationDebug userType="company" />
      </div>
    );
  }

  const { company, verification, nextSteps } = verificationData;

  return (
    <div className="space-y-6">
      {/* Status Overview */}
      <div className={`rounded-lg border-2 p-6 ${getStatusColor(company.verificationStatus)}`}>
        <div className="flex items-start justify-between">
          <div className="flex items-center space-x-3">
            {getStatusIcon(company.verificationStatus)}
            <div>
              <h3 className="text-lg font-semibold text-gray-900">
                Verification Status: {company.verificationStatus.replace('_', ' ').charAt(0).toUpperCase() + company.verificationStatus.replace('_', ' ').slice(1)}
              </h3>
              <p className="text-sm text-gray-600">
                Requested {company.daysSinceRequest} days ago • Priority: {company.verificationPriority.charAt(0).toUpperCase() + company.verificationPriority.slice(1)}
              </p>
              {company.verifiedAt && (
      ...
      {verification.canSubmitDocuments && (
        <div className="flex gap-3">
          ...
          {company.verificationStatus === 'pending' && verification.submittedDocuments >= verification.requiredDocumentsCount && (
            <button
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="space-y-4 max-h-64 overflow-y-auto">
            {verification.notes.map((note, index) => (
              <div key={index} className={`p-4 rounded-lg ${note.isFromAdmin ? 'bg-blue-50 border-l-4 border-blue-400' : 'bg-gray-50 border-l-4 border-gray-400'}`}>
                <div className="flex items-start justify-between">
                  ...
                  <div className="ml-4 flex-shrink-0">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      note.isFromAdmin ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                    }`}>
